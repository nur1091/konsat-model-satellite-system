...
The matrix in `data/requirements_traceability.csv` focuses on communication, ground control, electrical power, flight-software interfaces and the corrected RF analysis. It does not reproduce every competition requirement from the source reviews.
...
2. **Unit tests** - exercise each sensor, RTC, GPS, XBee radio and storage function independently.
...
7. **Integrated functional test** - verify commands, live plots, CSV output, reset recovery and deployment interlocks.
8. **Environmental tests** - perform vibration, shock and deployment tests only after electrical and functional closure.
...
- Sensor values and expected injected values
- Test distance, antenna orientation, obstacles and weather
- Payload and ground-station power state

## Current closure status

The available files support PDR/CDR design evidence. They do not support closure of range, endurance, software, deployment or environmental test requirements.
